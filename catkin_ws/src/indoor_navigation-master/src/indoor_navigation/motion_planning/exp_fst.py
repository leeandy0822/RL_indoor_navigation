...
from .exp_local_map_generator import ExpLocalMapGenerator

class ExpForwardSpanningTreePlanner:
    def __init__(self, sensor_dis, map_precision, r_n=10.0, n_samples=500, visualize=True):
        self.local_map = None
        self.local_map_generator = ExpLocalMapGenerator(sensor_dis, map_precision)
...
if __name__ == "__main__":
    rospy.init_node("exp_motion_planning_node")
    SENSOR_WIDTH = 6.0
    MAP_PRECISION = 0.1 # m
    R_N = 10.0
    N_SAMPLES = 500
    ...
    env = ExpPX4Env()
    state = env.reset()
    fst = ExpForwardSpanningTreePlanner(SENSOR_WIDTH, MAP_PRECISION, r_n=R_N, n_samples=N_SAMPLES, visualize=VISUALIZE)
